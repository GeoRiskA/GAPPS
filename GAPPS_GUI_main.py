...
Copyright:
    ...
    VUB — Vrije Universiteit Brussel (Brussels, BELGIUM)
        Physical Geography (FARD), Dpt. of Geography
    © 2020 – 2026
...
assets_folder = os.path.join(gapps_main_folder, "assets") # Access the logos and icons
...
from GAPPS_tools.GAPPS_Tool_03_AirPhoto_Reprojection import image_reprojection
from GAPPS_tools.GAPPS_Tool_04_AirPhotos_Resize import image_resampling_sharpening
...
# root.rowconfigure(28, weight=1)
    ...
    # Add the GAPPS logo
logo = Image.open(f"{assets_folder}/GAPPS_logo_forGUI.png")
...
#space_01 = tk.Label(root, text=" ", font=("Arial", 10, "bold")
#space_01.grid(row=5, column=0)
...
# --------------------------------------
    ...
    # Main label
label_02 = tk.Label(root, text="   Tool 2 – Fiducial Mark (FM) Detection", font=("Arial", 12, "bold")
, fg='black')
label_02.grid(row=11, column=0, columnspan=3, pady=5, sticky='w')

        # Sub-label 1
label_02a = tk.Label(root, text="         2.1. Select or create the new camera model", font=("Arial", 12, "italic"), fg='black')
label_02a.grid(row=12, column=0, columnspan=3, pady=5, sticky='w')
...
entry_camera.grid(row=14, column=0)
...
camera_option = tk.Label(root, text="Alternative option:", font=("Arial", 10), fg='black')
camera_option.grid(row=14, column=2, sticky='e')

add_camera_button = tk.Button(root, text="Add new camera model...", font=("Arial", 10, "bold")
, fg='black', command=launch_add_camera_gui)
add_camera_button.grid(row=14, column=3, sticky='w')

        # Sub-label 2
label_02b = tk.Label(root, text="         2.2. Create the FM templates", font=("Arial", 12, "italic"), fg='black')
label_02b.grid(row=16, column=0, columnspan=3, pady=10, sticky='w')
...
button_02b = tk.Button(root, text="Launch template creator...", font=("Arial", 10, "bold")
, command=fm_template_creator_gui, fg='black', width=20, height=1)
button_02b.grid(row=17, column=0)

        # Sub-label 3
label_02c = tk.Label(root, text="         2.3. FM detection (automatic, manual correction)", font=("Arial", 12, "italic"), fg='black')
...
entry_template_folder.grid(row=21, column=1, columnspan=2, sticky='w')
...
pValue_label.grid(row=22, column=0)
...
entry_p.grid(row=23, column=0)

        # Stripe location
stripe_label = tk.Label(root, text="Stripe location:", font=("Arial", 10), fg='black')
stripe_label.grid(row=22, column=1)
...
entry_stripe.grid(row=23, column=1)
...
auto_FM_detection_button = tk.Button(root, text="Run automatic FM detection", font=("Arial", 10, "bold")
, command=auto_FM_detection, fg='black', width=25, height=1)
auto_FM_detection_button.grid(row=22, column=3)
...
manual_FM_detection_button = tk.Button(root, text="Run manual FM correction", font=("Arial", 10, "bold")
, command=manual_FM_detection, fg='black', width=25, height=1)
manual_FM_detection_button.grid(row=23, column=3)
...
# ---------------------------------
    ...
    # Main label
label_03 = tk.Label(root, text="   Tool 3 – Image Reprojection", font=("Arial", 12, "bold")
, fg='black')
label_03.grid(row=26, column=0, columnspan=1, pady=5, sticky='w')
...
def resolution_list(self, camera):
    ...
    # resolution buttons
input_resol_label = tk.Label(root, text="Input resolution (dpi)", font=("Arial", 10), fg='black')
input_resol_label.grid(row=28, column=0)
resolution_value_list = ["300", "400", "600", "800", "900", "1200", "1500", "1600", "1800"]

chosen_input_res = tk.StringVar(root)
chosen_input_res.set(resolution_value_list[7]) # default value
entry_input_res = ttk.OptionMenu(root, chosen_input_res, *resolution_value_list)
entry_input_res.grid(row=29, column=0, sticky="nsew")

output_resol_label = tk.Label(root, text="Output resolution (dpi)", font=("Arial", 10), fg='black')
output_resol_label.grid(row=30, column=0)

chosen_output_res = tk.StringVar(root)
chosen_output_res.set(resolution_value_list[4])  # default value
entry_ouput_res = ttk.OptionMenu(root, chosen_output_res, *resolution_value_list)
entry_ouput_res.grid(row=31, column=0, sticky="nsew")

    # Sharpening options
clahe_label = tk.Label(root, text="Histogram calibration", font=("Arial", 10), fg='black')
clahe_label.grid(row=28, column=1, sticky="nsew")
...
entry_clahe.grid(row=29, column=1, sticky="nsew")

sharpening_intensity_label = tk.Label(root, text="Sharpening intensity", font=("Arial", 10), fg='black')
sharpening_intensity_label.grid(row=30, column=1)
...
entry_intensity.grid(row=31, column=1, sticky="nsew")
...
button_04.grid(row=31, column=3)

    # Create a horizontal separator
separator4 = ttk.Separator(root, orient='horizontal')
separator4.grid(row=32, column=0, columnspan=4, sticky='ew', pady=10)
...
# ----------------------------
    ...
    # Main label
label_05 = tk.Label(root, text="   Tool 5 – Image Masking", font=("Arial", 12, "bold"), fg='black')
label_05.grid(row=34, column=0, columnspan=3, pady=5, sticky='w')

    # Description
label_05_info = tk.Label(
    root,
    text="Create a corner mask from a reference image or from manually specified image dimensions.",
    font=("Arial", 10),
    fg='black'
)
label_05_info.grid(row=35, column=0, columnspan=3, pady=5, sticky='w')

    # Launch secondary GUI
def launch_mask_gui():
    script_path = os.path.join(gui_folder, "GAPPS_interface_create_mask.py")
    subprocess.Popen([sys.executable, script_path, input_img_folder, output_results_folder])

mask_button = tk.Button(
    root,
    text="Launch mask creator...",
    font=("Arial", 10, "bold"),
    command=launch_mask_gui,
    fg='black',
    width=25,
    height=2
)
mask_button.grid(row=37, column=0, pady=(5, 30))
...
copyright_label = tk.Label(root, text="© Royal Museum for Central Africa / Vrije Universiteit Brussel, 2020-2026. This software is released under the GPL-3.0 license.",
                         font=("Arial", 9, "italic"), fg="gray")
copyright_label.grid(row=44, column=0, columnspan=3, sticky='s')
